...
is
   ...
   procedure Mult_Limit (Value_1 : Long_Unsigned;
                         ...
                         Exp_2   : Natural) with
     Pre  => 2**Exp_1 <= Long_Unsigned'Last
               and 2**Exp_2 <= Long_Unsigned'Last
               and 0 <= Value_2
               and Value_1 <= 2**Exp_1
               and Value_2 <= 2**Exp_2,
   ...
   procedure Mult_Mono (X : Long_Unsigned;
                        Y : Long_Unsigned;
                        Z : Long_Unsigned) with
     Pre  => 0 <= Z and X <= Y,
     Post => Z * X <= Z * Y,
     Ghost;

   procedure Mult_Div_Id (X : Long_Unsigned;
                          Y : Long_Unsigned) with
     Pre  => 0 < Y,
     Post => X * Y / Y = X,
     Ghost;
   ...
   procedure Exp_Div (Base  : Long_Unsigned;
     ...
     Pre  => 0 < Base
               ...
               and then Base**(Exp_1 - Exp_2) <= Long_Unsigned'Last
               and then Base**Exp_2 <= Long_Unsigned'Last,
     Post => Base**Exp_1 / Base**Exp_2 = Base**(Exp_1 - Exp_2),
     Ghost;
