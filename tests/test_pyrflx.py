...
def test_aspect_checksum(echo_request_reply_message: MessageValue) -> None:
    ...
    def checksum_icmp(message: bytes, **kwargs) -> int:
        ...
        tag_first, checksum_first_minus_one = kwargs.get(
            "Tag'First .. Checksum'First - 1", (None, None)
        )
        identifier_first, data_last = kwargs.get("Identifier'First .. Data'Last", (None, None))
        checksum_length = kwargs.get("Checksum'Length", None)
        checksum_bytes = b""
        if tag_first is not None and checksum_first_minus_one is not None:
            checksum_bytes += message[tag_first : (checksum_first_minus_one + 1) // 8]
        if checksum_length:
            checksum_bytes += b"\x00" * (checksum_length // 8)
        if identifier_first is not None and data_last is not None:
        ...
        print(f"Länge Checksum Bytes {len(checksum_bytes.hex())}")
